...
Item {
    ...
    readonly property string scriptsDir: Quickshell.env("HOME") + "/.config/quickshell/components/network"
    ...
    Timer {
        ...
        onTriggered: { 
            if (window.connectingId === "" && Object.keys(window.busyTasks).length === 0) {
    ...
    Timer {
        id: wifiScanner
        interval: 12000
        running: window.isWindowVisible && window.activeMode === "wifi" && window.wifiPower === "on"
        repeat: true
    ...
    Process {
        ...
        running: window.isWindowVisible && window.activeMode === "bt" && window.btPower === "on" && window.connectingId === "" && Object.keys(window.busyTasks).length === 0
    }
